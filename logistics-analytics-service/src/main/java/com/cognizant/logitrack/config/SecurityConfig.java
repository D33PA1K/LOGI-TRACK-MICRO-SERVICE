    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http
                ...
                .exceptionHandling(exceptions -> exceptions
                        ...
                        // Match on the path, not just on GET: generating a report triggers a
                        // cross-service aggregation and writes a row, so it must be as
                        // restricted as reading one. Matching only HttpMethod.GET previously
                        // let any authenticated user (e.g. a DRIVER) POST a new report.
